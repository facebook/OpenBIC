...
static int temp_threshold_set_all_default(const struct shell *shell)
{
	...
	for (int i = 0; i < PLAT_TEMP_INDEX_THRESHOLD_TYPE_MAX; i++) {
		if (!set_plat_temp_threshold(i, &temperature, true, false)) {
...
static int cmd_temp_threshold_get_all(const struct shell *shell, size_t argc, char **argv)
{
	...
	for (int i = 0; i < PLAT_TEMP_INDEX_THRESHOLD_TYPE_MAX; i++) {
		...
		if (!get_plat_temp_threshold(i, &temperature)) {
...
static int cmd_temp_threshold_set(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (get_temp_threshold_type_enum(argv[1], &temp_index_threshold_type) == false) {
	...
	if (!set_plat_temp_threshold(temp_index_threshold_type, &temperature, is_default,
				     is_perm)) {
...
static void get_temp_index_threshold_type_name_(size_t idx, struct shell_static_entry *entry)
{
	uint8_t *name = NULL;
	get_temp_index_threshold_type_name((uint8_t)idx, &name);
...
SHELL_DYNAMIC_CMD_CREATE(temp_index_threshold_type_name, get_temp_index_threshold_type_name_);
