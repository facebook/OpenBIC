...
static int cmd_i2c_switch_get_all(const struct shell *shell, size_t argc, char **argv)
{
	...
	shell_print(shell, "MEDHA0_SW_EN : %d", gpio_get(MEDHA0_SW_EN));
	shell_print(shell, "MEDHA1_SW_EN : %d", gpio_get(MEDHA1_SW_EN));
...
static int cmd_i2c_switch_set_all(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (set_val == 1) {
		...
		gpio_set(MEDHA0_SW_EN, 1);
		gpio_set(MEDHA1_SW_EN, 1);
	...
	gpio_set(MEDHA0_SW_EN, 0);
	gpio_set(MEDHA1_SW_EN, 0);
...
SHELL_CMD_REGISTER(i2c_switch_control, &sub_i2c_switch_control_cmds,
		   "I2C switch control (HAMSA/MEDHA0/MEDHA1)", NULL);
