...
#define name_gpioA                                                                                 \
	gpio_name_to_num(FM_BMC_PCH_SCI_LPC_N) gpio_name_to_num(FM_BIOS_POST_CMPLT_BMC_N)          \
		gpio_name_to_num(FM_SLPS3_R_N) gpio_name_to_num(IRQ_BMC_PCH_SMI_LPC_R_N)           \
			gpio_name_to_num(IRQ_UV_DETECT_N) gpio_name_to_num(FM_UV_ADR_TRIGGER_EN)   \
				gpio_name_to_num(IRQ_SMI_ACTIVE_BMC_N)                             \
					gpio_name_to_num(HSC_SET_EN)
#define name_gpioB                                                                                 \
	gpio_name_to_num(FM_BIC_RST_RTCRST) gpio_name_to_num(RST_USB_HUB_N_R)                      \
		gpio_name_to_num(A_P3V_BAT_SCALED_EN) gpio_name_to_num(FM_CPU_FIVR_FAULT_LVT3_N)   \
			gpio_name_to_num(DBP_SYSPWROK_R) gpio_name_to_num(FM_SLPS4_R_N)            \
...
#define name_gpioD                                                                                 \
	gpio_name_to_num(PWRBTN_N) gpio_name_to_num(RST_BMC_R_N)                                   \
		gpio_name_to_num(IRQ_BMC_PRDY_NODE_OD_N) gpio_name_to_num(BMC_READY)               \
...
#define name_gpioE                                                                                 \
	gpio_name_to_num(PWRGD_CPU_LVC3_R) gpio_name_to_num(FM_PCH_BMC_THERMTRIP_N)                \
		gpio_name_to_num(HSC_DETECT0) gpio_name_to_num(HSC_DETECT1)                        \
			gpio_name_to_num(HSC_DETECT2) gpio_name_to_num(FM_CPU_MSMI_CATERR_LVT3_N)  \
...
#define name_gpioF                                                                                 \
	gpio_name_to_num(IRQ_BMC_PCH_NMI_R) gpio_name_to_num(IRQ_SML1_PMBUS_ALERT_N)               \
		gpio_name_to_num(IRQ_NMI_EVENT_R_N) gpio_name_to_num(FM_BMC_DEBUG_ENABLE_N)        \
			gpio_name_to_num(DBP_PRESENT_R2_N) gpio_name_to_num(FM_FAST_PROCHOT_EN_N)  \
				gpio_name_to_num(FM_BMC_ONCTL_R_N)                                 \
					gpio_name_to_num(FM_BMC_CPU_PWR_DEBUG_N)
#define name_gpioG                                                                                 \
	gpio_name_to_num(FM_PEHPCPU_INT) gpio_name_to_num(FM_MRC_DEBUG_EN)                         \
		gpio_name_to_num(FAST_PROCHOT_N) gpio_name_to_num(FM_JTAG_TCK_MUX_SEL)             \
			gpio_name_to_num(BMC_JTAG_SEL) gpio_name_to_num(FM_CPU_ERR0_LVT3_N)        \
...
#define name_gpioH                                                                                 \
	gpio_name_to_num(RST_RSMRST_BMC_N) gpio_name_to_num(FM_MP_PS_FAIL_N)                       \
		gpio_name_to_num(FM_MEM_THERM_EVENT_LVT3_N)                                        \
			gpio_name_to_num(IRQ_SML0_ALERT_MUX_R_N) gpio_name_to_num(Reserve_GPIOH4)  \
				gpio_name_to_num(Reserve_GPIOH5) gpio_name_to_num(Reserve_GPIOH6)  \
...
#define name_gpioM                                                                                 \
	gpio_name_to_num(Reserve_GPIOM0) gpio_name_to_num(BOARD_ID3)                               \
		gpio_name_to_num(Reserve_GPIOM2) gpio_name_to_num(FM_THERMTRIP_DLY_TO_PCH)         \
...
#define name_gpioN                                                                                 \
	gpio_name_to_num(SGPIO_BMC_CLK_R) gpio_name_to_num(SGPIO_BMC_LD_N)                         \
...
#define name_gpioO                                                                                 \
	gpio_name_to_num(Reserve_GPIOO0) gpio_name_to_num(Reserve_GPIOO1)                          \
		gpio_name_to_num(Reserve_GPIOO2) gpio_name_to_num(RST_PLTRST_BMC_N)                \
			gpio_name_to_num(Reserve_GPIOO4) gpio_name_to_num(JTAG_BMC_NTRST_R_N)      \
...
#define name_gpioQ                                                                                 \
	gpio_name_to_num(IRQ_HSC_ALERT2_N) gpio_name_to_num(Reserve_GPIOQ1)                        \
