...
#define MB_THROTTLE_DELAY_US 20
...
void ISR_MB_THROTTLE()
{
	...
	/* <0 is real error */
        if (k_work_cancel_delayable(&mb_throttle_work) < 0) {          
            LOG_ERR("Cancel MB_THROTTLE Task error\n");
        }

        k_work_schedule_for_queue(&plat_work_q, &mb_throttle_work, K_USEC(MB_THROTTLE_DELAY_US));
