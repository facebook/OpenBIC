...
#define POSTCODE_PREFIX_ASP_FMC 0xAA000000ul
#define POSTCODE_PREFIX_ASP_TEE 0xED000000ul
...
#define GET_LOW_BYTE(code) ((code) & 0xFFul)
#define GET_24BIT_VALUE(code) ((code) & 0x00FFFFFF)

#define POSTCODE_LOW_BYTE_MAX 0x009F
...
/* Accepts 0x{EE,AA,ED}xxxx00 - 0x{EE,AA,ED}xxxx9F; middle two bytes are don't-care */
static bool validate_low_byte_range(uint32_t postcode)
{
	return GET_LOW_BYTE(postcode) <= POSTCODE_LOW_BYTE_MAX;
}
...
void pcc_platform_filter_init(void)
{
	...
	LOG_DBG("Filter arrays validated (ABL: %zu, AGESA: %zu)",
		ARRAY_SIZE(abl_filter_error_codes), ARRAY_SIZE(agesa_filter_error_codes));
#endif
}
...
bool pcc_platform_filter_postcode(uint32_t postcode)
{
	...
	case POSTCODE_PREFIX_ASP_FMC:
	case POSTCODE_PREFIX_ASP_TEE:
		return validate_low_byte_range(postcode);

	case POSTCODE_PREFIX_UNEXPECTED:
		if (postcode == POSTCODE_EXCLUDE_ZERO || postcode == POSTCODE_EXCLUDE_ABCD) {
			return false;
		}
		return true;
