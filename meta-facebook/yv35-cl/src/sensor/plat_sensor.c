...
void pal_fix_Sensorconfig()
{
  ...
  if ( get_bic_class() ) {
    // fix usage when fix_C2Sensorconfig_table is defined
    fix_SensorCfg_num = sizeof( fix_C2Sensorconfig_table ) / sizeof( fix_C2Sensorconfig_table[0] );
    while ( fix_SensorCfg_num ) {
      add_Sensorconfig ( fix_C2Sensorconfig_table[ fix_SensorCfg_num - 1 ] );
      fix_SensorCfg_num--;
    }
  }
  if ( get_1ou_status() ) {
    // fix usage when fix_1ouSensorconfig_table is defined
    fix_SensorCfg_num = sizeof( fix_1ouSensorconfig_table ) / sizeof( fix_1ouSensorconfig_table[0] );
    while ( fix_SensorCfg_num ) {
      add_Sensorconfig ( fix_1ouSensorconfig_table[ fix_SensorCfg_num - 1 ] );
      fix_SensorCfg_num--;
    }
  }
  if ( get_2ou_status() ) {
    // fix usage when fix_DVPSensorconfig_table is defined
    fix_SensorCfg_num = sizeof( fix_DVPSensorconfig_table ) / sizeof( fix_DVPSensorconfig_table[0] );
    while ( fix_SensorCfg_num ) {
      add_Sensorconfig ( fix_DVPSensorconfig_table[ fix_SensorCfg_num - 1 ] );
      fix_SensorCfg_num--;
    }
  }
  if ( SensorCfg_num != SDR_NUM ) {
