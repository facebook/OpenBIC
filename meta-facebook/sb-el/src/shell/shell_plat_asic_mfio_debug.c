...
static const asic_mifo_pin_map_t mfio_list[] = {
	{ "HAMSA_MFIO12", HAMSA_MFIO12_13_14_CTRL, 2, HAMSA_MFIO12_13_14_INPUT, 2, HAMSA_MFIO_REG,
	  7 },
	{ "HAMSA_MFIO13", HAMSA_MFIO12_13_14_CTRL, 1, HAMSA_MFIO12_13_14_INPUT, 1, HAMSA_MFIO_REG,
	  6 },
	{ "HAMSA_MFIO14", HAMSA_MFIO12_13_14_CTRL, 0, HAMSA_MFIO12_13_14_INPUT, 0, HAMSA_MFIO_REG,
	  5 },
	{ "NUWA0_MFIO12", NUWA_MFIO12_13_14_CTRL, 5, NUWA_MFIO12_13_14_INPUT, 5, NUWA_MFIO_REG, 5 },
	{ "NUWA0_MFIO13", NUWA_MFIO12_13_14_CTRL, 4, NUWA_MFIO12_13_14_INPUT, 4, NUWA_MFIO_REG, 4 },
	{ "NUWA0_MFIO14", NUWA_MFIO12_13_14_CTRL, 3, NUWA_MFIO12_13_14_INPUT, 3, NUWA_MFIO_REG, 3 },
	{ "NUWA1_MFIO12", NUWA_MFIO12_13_14_CTRL, 2, NUWA_MFIO12_13_14_INPUT, 2, NUWA_MFIO_REG, 2 },
	{ "NUWA1_MFIO13", NUWA_MFIO12_13_14_CTRL, 1, NUWA_MFIO12_13_14_INPUT, 1, NUWA_MFIO_REG, 1 },
	{ "NUWA1_MFIO14", NUWA_MFIO12_13_14_CTRL, 0, NUWA_MFIO12_13_14_INPUT, 0, NUWA_MFIO_REG, 0 },
};
...
static int cmd_mfio_get_all(const struct shell *shell, size_t argc, char **argv)
{
	...
	for (int i = 0; i < ARRAY_SIZE(unassigned_mfio_list); i++) {
		...
		if ((config_value >> unassigned_mfio_list[i].bit) & 1)
			shell_print(shell, "%-20s|%-15s|%-10d", unassigned_mfio_list[i].name,
				    "Input", (input_value >> unassigned_mfio_list[i].bit) & 1);
		else
			shell_print(shell, "%-20s|%-15s|%-10d", unassigned_mfio_list[i].name,
				    "Output", (output_value >> unassigned_mfio_list[i].bit) & 1);
	}
...
static int cmd_set_mfio_io(const struct shell *shell, size_t argc, char **argv)
{
	...
	for (int i = 0; i < ARRAY_SIZE(mfio_list); i++) {
		if (!strcmp(mfio_list[i].name, argv[1])) {
			...
			shell_print(shell, "set %s as : %s", mfio_list[i].name,
				    (set_value == 0) ? "Input" : "Output");
			return 0;
		}
	}
	...
	for (int i = 0; i < ARRAY_SIZE(unassigned_mfio_list); i++) {
		if (!strcmp(unassigned_mfio_list[i].name, argv[1])) {
			if (!tca6424a_i2c_write_bit(
				    port_map[unassigned_mfio_list[i].port].config_reg,
				    unassigned_mfio_list[i].bit, set_value)) {
				shell_error(shell, "write MFIO config to ioexp failed");
				return -1;
			}
			shell_print(shell, "set %s as : %s", unassigned_mfio_list[i].name,
				    (set_value == 0) ? "Output" : "Input");
			return 0;
		}
	}
error:
...
static int cmd_set_mfio_value(const struct shell *shell, size_t argc, char **argv)
{
	...
	for (int i = 0; i < ARRAY_SIZE(mfio_list); i++) {
		if (!strcmp(mfio_list[i].name, argv[1])) {
			...
			if (!set_cpld_bit(mfio_list[i].output_reg, mfio_list[i].output_bit,
					  set_value)) {
	...
	for (int i = 0; i < ARRAY_SIZE(unassigned_mfio_list); i++) {
		if (!strcmp(unassigned_mfio_list[i].name, argv[1])) {
			tca6424a_i2c_read(port_map[unassigned_mfio_list[i].port].config_reg,
					  &config_value, 1);
			if ((config_value >> unassigned_mfio_list[i].bit) & 1) {
				shell_error(shell, "Can't set value to Input port");
				return -1;
			...
			if (!tca6424a_i2c_write_bit(
				    port_map[unassigned_mfio_list[i].port].output_reg,
				    unassigned_mfio_list[i].bit, set_value))
				return -1;
			shell_print(shell, "set %s value to: %d", unassigned_mfio_list[i].name,
				    set_value);
			return 0;
		}
	}
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD(value, &mifo_name, "set value <mfio_name> 1/0, 1 = high, 0 = low",
		  cmd_set_mfio_value),
	SHELL_SUBCMD_SET_END);
...
SHELL_CMD_REGISTER(asic_mfio_debug, &sub_asic_mfio_debug_cmds, "asic_mfio_debug set/get commands",
		   NULL);
