...
100:HAMSA
*/
enum jtag_mux { MUX_HAMSA = 0, MUX_NUWA_0, MUX_NUWA_1, MUX_OWL_E, MUX_OWL_W, MUX_JTAG_MUX_MAX };

LOG_MODULE_REGISTER(plat_jtag_mux_shell);

void set_cmd_jtag_mux(const struct shell *shell, size_t argc, char **argv)
{
	//jtag_mux set <NUWA_0 | NUWA_1 | OWL_E | OWL_W | HAMSA>
	// if argc not match
	if (argc != 1) {
		shell_warn(shell, "Help: jtag_mux set <NUWA_0 | NUWA_1 | OWL_E | OWL_W | HAMSA>");
		return;
	}
	uint8_t write_in_data = 0;
	if (strcmp(argv[0], "NUWA_0") == 0) {
		write_in_data = MUX_NUWA_0;
		plat_write_cpld(ASIC_JTAG_MUX_SEL, &write_in_data);
		shell_info(shell, "jtag_mux set to NUWA_0");
	} else if (strcmp(argv[0], "NUWA_1") == 0) {
		write_in_data = MUX_NUWA_1;
		plat_write_cpld(ASIC_JTAG_MUX_SEL, &write_in_data);
		shell_info(shell, "jtag_mux set to NUWA_1");
		...
		shell_error(
			shell,
			"Type wrong sub command, Help: jtag_mux set <NUWA_0 | NUWA_1 | OWL_E | OWL_W | HAMSA>");
	}
}
...
void jtag_mux_get_cmds(const struct shell *shell, size_t argc, char **argv)
{
	...
	JTAG MUX : <NUWA_0 | NUWA_1 | OWL_E | OWL_W | HAMSA>
	*/
...
SHELL_STATIC_SUBCMD_SET_CREATE(jtag_mux_set_cmds,
			       SHELL_CMD(NUWA_0, NULL, "NUWA_0", set_cmd_jtag_mux),
			       SHELL_CMD(NUWA_1, NULL, "NUWA_1", set_cmd_jtag_mux),
