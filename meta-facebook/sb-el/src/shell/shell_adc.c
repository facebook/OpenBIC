...
void shell_adc_get_buf_raw(const struct shell *shell, uint8_t idx)
{
	...
	shell_warn(shell, "LV%d NUWA%d ADC buf_raw (len=%d)(unit=V): ", lv, idx % 2, len);
	...
	shell_print(shell, "\n");
	shell_warn(shell, "LV%d NUWA%d VR Volt buf_raw (len=%d)(unit=V)): ", lv, idx % 2, len);
...
void shell_adc_get_buf(const struct shell *shell, uint8_t idx)
{
	...
	shell_warn(shell, "LV%d NUWA%d CURR (len=%d)(unit=A)): ", lv, idx % 2, len);
	for (uint16_t i = 0; i < len; i++) {
	...
	shell_warn(shell, "LV%d NUWA%d PWR (len=%d)(unit=W)): ", lv, idx % 2, len);
	uint16_t *vr_buf = get_vr_buf(idx);
	for (uint16_t i = 0; i < len; i++) {
	...
	shell_print(shell, "");
	// VR voltage buf
	shell_warn(shell, "LV%d NUWA%d VR VOLT (len=%d)(unit=V)): ", lv, idx % 2, len);
	for (uint16_t i = 0; i < len; i++) {
	...
	shell_print(shell, "");
	// ADC voltage buf
	shell_warn(shell, "LV%d NUWA%d ADC VOLT (len=%d)(unit=V): ", lv, idx % 2, len);
	for (uint16_t i = 0; i < len; i++) {
