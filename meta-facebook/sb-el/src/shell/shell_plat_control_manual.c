...
static int cmd_enable_manual_control_strap(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (!set_cpld_bit(CPLD_OFFSE_MANUAL_CONTROL_STRAP, MANUAL_CONTROL_STRAP_BIT,
			  ENABLE_MANUAL_CONTROL_STRAP)) {
		shell_error(shell, "write manual control strap to CPLD failed");
		return -1;
	}

	if (!plat_read_cpld(CPLD_OFFSE_MANUAL_CONTROL_STRAP, &reg_status, 1)) {
	...
	shell_print(shell, "STRAP_HIZ_EN : %d (low active)",
		    reg_status & MANUAL_CONTROL_STRAP_BIT_MASK);
	return 0;
}
...
SHELL_CMD_REGISTER(strap_control_manual, &sub_set_strap_control_cmds,
		   "strap_control_manual <enable | disable | get>", NULL);
