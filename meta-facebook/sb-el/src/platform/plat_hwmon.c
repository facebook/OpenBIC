...
void init_pwm_dev(void)
{
	...
	pwm2_dev = device_get_binding("PWM_2");
	pwm6_dev = device_get_binding("PWM_6");

	if (pwm2_dev == NULL)
		LOG_ERR("FAN_2 PWM2 init failed due to device not found");
...
{
	if (pwm2_dev == NULL) {
		LOG_ERR("FAN_2 PWM2 dev not found!");
		return 1;
	}
	...
	if (port != PWM_PORT2 && port != PWM_PORT6) {
	...
	pwm_pin_set_cycles(pwm2_dev, port, PWM_PERIOD, (PWM_PERIOD * duty / 100), 0);
