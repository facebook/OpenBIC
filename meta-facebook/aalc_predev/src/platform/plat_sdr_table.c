...
SDR_Full_sensor plat_sb_temp_table[] = {
	{
...
const int SB_TEMP_SDR_TABLE_SIZE = ARRAY_SIZE(plat_sb_temp_table);
...
uint8_t pal_get_extend_sdr()
{
	...
	if (temp_module < TEMP_MODULE_UNKNOWN)
		extend_sdr_table_size += SB_TEMP_SDR_TABLE_SIZE;
...
void pal_extend_full_sdr_table()
{
	...
	uint8_t temp_module = get_temp_module();
	if (temp_module < TEMP_MODULE_UNKNOWN)
		for (int index = 0; index < SB_TEMP_SDR_TABLE_SIZE; index++)
			add_full_sdr_table(plat_sb_temp_table[index]);
}