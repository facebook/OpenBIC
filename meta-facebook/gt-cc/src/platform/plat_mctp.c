...
{
	LOG_INF("Setting MCTP endpoint for devices");
	...
	for (uint8_t i = 0; i < ARRAY_SIZE(mctp_route_tbl); i++) {
		...
		ret = get_mctp_info(p->endpoint, &mctp_inst, &msg.ext_params);
		if (ret != MCTP_SUCCESS) {
			LOG_ERR("Get mctp route info fail for endpoint 0x%x", p->endpoint);
			continue;
		}

		if (!mctp_inst) {
			LOG_ERR("mctp_inst is NULL for endpoint 0x%x", p->endpoint);
			continue;
		}

		/** Set eid directly without checking **/
		...
		ret = mctp_ctrl_read(mctp_inst, &msg, (uint8_t *)&set_eid_resp,
				     sizeof(set_eid_resp));
		if (ret != MCTP_SUCCESS) {
			LOG_ERR("Fail to set eid, endpoint: 0x%x, bus: %d, ret: 0x%x", p->endpoint,
				p->bus, ret);
			continue;
		}
		LOG_INF("Successfully set eid 0x%x on bus %d", p->endpoint, p->bus);

		k_msleep(50); // wait for the device to process the set eid command

		/** Read back eid to verify **/
		memset(&msg, 0, sizeof(mctp_ctrl_msg));
		msg.ext_params.type = MCTP_MEDIUM_TYPE_SMBUS;
		msg.ext_params.smbus_ext_params.addr = p->addr;
		...
		ret = mctp_ctrl_read(mctp_inst, &msg, (uint8_t *)&get_eid_resp,
				     sizeof(get_eid_resp));
		if (ret != MCTP_SUCCESS) {
			LOG_ERR("Fail to get eid after setting, endpoint: 0x%x, bus: %d, ret: 0x%x",
				p->endpoint, p->bus, ret);
			continue;
		}

		if (get_eid_resp.eid == p->endpoint) {
			LOG_INF("Verified: Endpoint 0x%x has correct eid on bus %d", p->endpoint,
				p->bus);
		} else {
			LOG_WRN("Warning: Endpoint 0x%x eid mismatch, expected 0x%x but got 0x%x",
				p->endpoint, p->endpoint, get_eid_resp.eid);
		}
	}
}
...
static void get_dev_firmware_resp_handler(void *args, uint8_t *buf, uint16_t len)
{
	...
	if (response->completion_code != PLDM_SUCCESS) {
		LOG_ERR("Failed to get firmware parameters for NIC%d, completion_code = 0x%x",
			nic_index, response->completion_code);
		return;
	}

	/* Sanity check for version string length */
	if (response->active_comp_image_set_ver_str_len == 0) {
		LOG_WRN("NIC%d firmware version string length is 0", nic_index);
		return;
	}

	if (response->active_comp_image_set_ver_str_len > 256) {
		LOG_ERR("NIC%d firmware version string length too large: %d, possibly corrupted data",
			nic_index, response->active_comp_image_set_ver_str_len);
		return;
	}

	LOG_DBG("NIC%d allocating %d bytes for firmware version", nic_index,
	...
	if (!nic_vesion[nic_index].ptr) {
		LOG_ERR("The buffer of NIC%d version memory allocate failed, requested size: %d",
			nic_index, response->active_comp_image_set_ver_str_len);
		return;
	}
...
void check_nic_config(void)
{
	...
	if (strncmp(nic_manufacturer, NVIDIA_NIC_MANUFACTURER,
		...
		bool found_name = get_first_nic_name(nic_name, sizeof(nic_name));
		if (found_name) {
			LOG_INF("First NIC name found: %s", nic_name);
			if (strstr(nic_name, "VULCANO") != NULL) {
				LOG_INF("VULCANO NIC detected based on NIC name");
				config = NIC_CONFIG_VULCANO;
			} else {
				LOG_INF("Unknown AMD NIC type based on NIC name, default to POLLARA config");
				config = NIC_CONFIG_POLLARA;
			}
		} else {
			LOG_INF("No NIC name found for AMD NIC, default to POLLARA config");
			config = NIC_CONFIG_POLLARA;
		}
	} else {
	...
	nic_config = config;
	LOG_INF("NIC config is %d, 0: UNKNOWN, 1: CX7, 2: IB_CX7, 3: THOR2, 4: POLLARA, 5: VULCANO",
		nic_config);
...
void send_cmd_to_dev_handler(struct k_work *work)
{
	...
	update_nic_sensor_config_for_vulcano();
	nic_drive_reinit_for_vulcano();
	nic_optics_drive_reinit_for_vulcano();
