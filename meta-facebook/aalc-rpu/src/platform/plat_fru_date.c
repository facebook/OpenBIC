...
void store_ascii_to_result(char *str, uint8_t *result, int *index) {
    for (int i = 0; str[i] != '\0'; i++) {
        result[(*index)++] = (uint8_t)str[i];  // Convert character to ASCII value and store it
    }
}

// Convert the year, month, day, hour, minute, etc., to strings and store them in the result array
void store_time_to_result(int year, int month, int day, int hour, int minute, int weekday, uint8_t *result) {
...
void calculate_time_in_hex(long long total_minutes, uint8_t *result) {
    ...
    store_time_to_result(year, month, day, hour, minute, weekday, result);
}
...
uint8_t get_fru_date(uint16_t *data, uint16_t *return_data) 
{
    ...
    total_minutes = ((data[2] & 0xFF) << 6) + ((data[1] & 0xFF) << 4) + (data[0] & 0xFF);
    
    // Get the number of minutes from the user (0xe647ae is a sample input)
    total_minutes += MINUTES_FROM_1970_1996;

    // Calculate the time and store it in the result array
    calculate_time_in_hex(total_minutes, result);
