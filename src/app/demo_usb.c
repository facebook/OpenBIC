...
static void usb_write_task(void *argv)
{
	char *tx_buff = "aspeed bulk test \n\r";
	int snlen = strlen(tx_buff);
...
{
	char option;
	bool run = 0, delete = 0, exec = 0, stress = 0;
	optind = 0;
	while ((option = getopt(argc, argv, "hreds")) != (char)-1) {
		switch (option) {
			case 'h':
			case '?':
	...
	if (run) {
		...
		tid_taskA = osThreadNew(usb_write_task, NULL, &tattr_taskA);
