...
{
    aspeed_g_pwm_tach_priv_t *priv = (aspeed_g_pwm_tach_priv_t *)g_pwm_tach->private;
	pwm_tach_register_t *pwm_tach_register = ( volatile pwm_tach_register_t * ) g_pwm_tach->base;
	...
	aspeed_irq_register(Tach_IRQn, (uint32_t)aspeed_tach_isr, (void *)g_pwm_tach);
...
static void aspeed_pwm_ctrl(pwm_t *obj, uint8_t ctrl)
{
	...
	aspeed_g_pwm_tach_priv_t *g_priv = (aspeed_g_pwm_tach_priv_t *)parent->private;
	pwm_tach_register_t *pwm_tach_register = ( volatile pwm_tach_register_t * ) pwm->base;
	uint32_t pwm_channel = priv->pwm_channel;
	pwm_tach_gather_register_t *pwm_tach_gather = &pwm_tach_register->pwm_tach_gather[pwm_channel];
...
hal_status_t aspeed_pwm_init(pwm_t *obj)
{
	...
	pwm_tach_gather->pwm_general.fields.enable_pwm_pin = 0x1;
	pwm_tach_gather->pwm_general.fields.enable_pwm_clock = 0x1;
	pwm_tach_gather->pwm_general.fields.enable_pwm_duty_load_as_wdt = priv->wdt_reload_enable;
	pwm_tach_gather->pwm_general.fields.load_selection_of_duty_as_wdt = LOAD_SEL_FALLING;

	pwm_tach_gather->pwm_duty_cycle.fields.pwm_rising_point = 0;
	pwm_tach_gather->pwm_duty_cycle.fields.pwm_falling_point = (DEFAULT_PWM_PERIOD * priv->default_duty / 100);
	pwm_tach_gather->pwm_duty_cycle.fields.pwm_rising_falling_point_as_wdt = (DEFAULT_PWM_PERIOD * priv->wdt_reload_duty / 100);
...
static void aspeed_tach_isr(void)
{
	...
	aspeed_g_pwm_tach_priv_t *g_priv = (aspeed_g_pwm_tach_priv_t *)g_pwm_tach->private;
	pwm_tach_register_t *pwm_tach_register = ( volatile pwm_tach_register_t * ) g_pwm_tach->base;
...
static uint32_t aspeed_tach_get_sample_period(uint8_t pulse_pr, uint32_t min_rpm)
{
...
void aspeed_tach_int_cb_hook(tach_t *obj, uint32_t rpm_threshold, uint8_t cmp_more_less, callback cb, void *para)
{
	...
	pwm_tach_register_t *pwm_tach_register = ( volatile pwm_tach_register_t * ) tach->base;
	uint32_t tach_channel = priv->tach_channel;
	pwm_tach_gather_register_t *pwm_tach_gather = &pwm_tach_register->pwm_tach_gather[tach_channel];
	aspeed_tach_adapter_t *tach_adapter = &priv->adapter;
	uint32_t threshold;

	log_debug("cb(para) = 0x%08x(0x%08x)\n", cb, para);
	if (priv->tach_freq > (0xffffffff / 60))
		threshold = ((priv->tach_freq) / (priv->fan_pulse_pr * rpm_threshold)) * 60;
	else
		threshold = ((priv->tach_freq * 60) / (priv->fan_pulse_pr * rpm_threshold));
	log_debug("rpm_threshold = %d => tach threshold = %d\n", rpm_threshold, threshold);
	...
	pwm_tach_gather->tach_general.fields.inverse_tach_limit_comparison = cmp_more_less;
...
uint32_t aspeed_tach_get_rpm(tach_t *obj)
{
	...
	log_debug("rpm = (%d / %d * %d) * 60\n", priv->tach_freq, fan_pulse_pr, tach_value);
	if (tach_value){
		if (priv->tach_freq > (0xffffffff / 60))
