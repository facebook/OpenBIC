...
#define	AST_I2CS_ADDR3_CLEAR		(AST_I2CS_ADDR3_MASK|AST_I2CS_ADDR3_ENABLE|AST_I2CS_ADDR3_MBX_TYPE(0xff))
#define	AST_I2CS_ADDR2_CLEAR		(AST_I2CS_ADDR2_MASK|AST_I2CS_ADDR2_ENABLE|AST_I2CS_ADDR2_MBX_TYPE(0xff))
#define	AST_I2CS_ADDR1_CLEAR		(AST_I2CS_ADDR1_MASK|AST_I2CS_ADDR1_ENABLE|AST_I2CS_ADDR1_MBX_TYPE(0xff))
...
/* new for i2c snoop */
//#define PRINT_SNOOP
#define ALIGN_SNOOP  7 
#define ALIGN_MAILBOX  7 

#define AST_I2CS_SNOOP_DMA_WPT	0x50
#define AST_I2CS_SNOOP_DMA_RPT		0x58

//#define AST_I2CS_MAILBOX_OFFSET	0x60
...
//#define ASPEED_I2C_W_DUMP

#ifdef ASPEED_I2C_W_DUMP
#define I2C_W_R(value, addr) printf("  dw %x %x\n",addr,value); writel(value, addr);
#else
#define I2C_W_R(value, addr) writel(value, addr);
#endif
...
#endif
	//TODO define
	I2C_W_R(0x16, base + 0x0C);
	//writel(0x16, base + 0x0C);	
	I2C_W_R(0x03020100, base + 0x10);
	//writel(0x03020100, base + 0x10);
...
int aspeed_i2c_master_irq(i2c_t *obj)
{
	...
	if (AST_I2CM_BUS_RECOVER_FAIL & sts) {
		...
		I2C_W_R(AST_I2CM_BUS_RECOVER_FAIL, device->base + AST_I2CM_ISR);
		if (obj->bus_recover)
			obj->bus_recover = 0;
		else
	...
	if (AST_I2CM_BUS_RECOVER & sts) {
		...
		I2C_W_R(AST_I2CM_BUS_RECOVER, device->base + AST_I2CM_ISR);
		obj->cmd_err = 0;
		if (obj->bus_recover)
			obj->bus_recover = 0;
		else 
	...
	if (AST_I2CM_SMBUS_ALT & sts) {
		...
		//Disable ALT INT
		I2C_W_R(readl(device->base + AST_I2CM_IER) &
			      ~AST_I2CM_SMBUS_ALT, 
			      device->base + AST_I2CM_IER);
		I2C_W_R(AST_I2CM_SMBUS_ALT, device->base + AST_I2CM_ISR);
	...
	if(obj->cmd_err) {
		...
		I2C_W_R(AST_I2CM_PKT_DONE, device->base + AST_I2CM_ISR);
		return 1;
	}
			...
			case AST_I2CM_TX_ACK | AST_I2CM_NORMAL_STOP:
				...
				if (obj->master_xfer_cnt == obj->len) {
					...
					if (obj->mode == DMA_MODE) {
						...
						I2C_W_R(AST_I2CM_SET_TX_DMA_LEN(xfer_len - 1), device->base + AST_I2CM_DMA_LEN);
						I2C_DBUG("next tx xfer_len: %d, offset %d \n", xfer_len, obj->master_xfer_cnt);
						
						I2C_DBUG("M : t2\n");

//						I2C_W_R(i2c_bus->master_dma_addr + i2c_bus->master_xfer_cnt, device->base + AST_I2CM_TX_DMA);
					} else if (obj->mode == BUFF_MODE) {
						...
						if (--i % 4 != 3)
							I2C_W_R(*(uint32_t *)wbuf,
								   obj->buf_base + i - (i % 4));
						I2C_W_R(AST_I2CC_SET_TX_BUF_LEN(xfer_len), AST_I2CC_BUFF_CTRL);
					} else {
						//byte
						#if 0
						...
						I2C_W_R(msg->buf[i2c_bus->master_xfer_cnt], AST_I2CC_STS_AND_BUFF);
						#endif
					}
					I2C_DBUG("next tx cmd: %x\n", cmd);					
					I2C_W_R(cmd, device->base + AST_I2CM_CMD_STS);
				}
				break;
			...
			case AST_I2CM_RX_DONE | AST_I2CM_NORMAL_STOP:				
				...
				if (obj->master_xfer_cnt == obj->len) {
					...
					if (obj->mode == DMA_MODE) {
						...
						I2C_W_R(AST_I2CM_SET_RX_DMA_LEN(xfer_len - 1), device->base + AST_I2CM_DMA_LEN);
#ifdef AST1030
						I2C_W_R((uint32_t) (obj->buf + obj->master_xfer_cnt) | 0x80000000, device->base + AST_I2CM_RX_DMA);
#else
						I2C_W_R((uint32_t) obj->buf + obj->master_xfer_cnt, device->base + AST_I2CM_RX_DMA);
#endif
						...
						I2C_W_R(AST_I2CC_SET_RX_BUF_LEN(xfer_len), device->base + AST_I2CC_BUFF_CTRL);
					} else {
	...
	if(readl(device->base + AST_I2CM_ISR)) {
		...
		I2C_W_R(readl(device->base + AST_I2CM_ISR), device->base + AST_I2CM_ISR);
	}
		...
		switch (sts) {
			case AST_I2CS_SLAVE_MATCH | AST_I2CS_STOP:
				...
				if (obj->mode == DMA_MODE) {
					...
					I2C_W_R(AST_I2CS_SET_RX_DMA_LEN(I2C_SLAVE_BUFF_SIZE), device->base + AST_I2CS_DMA_LEN);
					//dma addr xxxx
				} else if (obj->mode == BUFF_MODE) {
					cmd |= AST_I2CS_RX_BUFF_EN;
					I2C_W_R(AST_I2CC_SET_RX_BUF_LEN(obj->buf_size), device->base + AST_I2CC_BUFF_CTRL);
				} else {
					cmd &= ~AST_I2CS_PKT_MODE_EN;
				}
				I2C_W_R(cmd, device->base + AST_I2CS_CMD_STS);
				break;
...
//					I2CS_DBUG("idx : %d \n ", obj->fifo_rx_idx);
					...
					if(sts & AST_I2CS_STOP) {
						if(((obj->fifo_rx_idx + 1) % I2C_SLAVE_MQ_FIFO_SIZE) == (obj->fifo_fetch_idx)) {
							...
							I2C_W_R(0, device->base + AST_I2CS_IER);
						} else {
							obj->fifo_rx_idx++;
							obj->fifo_rx_idx %= I2C_SLAVE_MQ_FIFO_SIZE;
//							I2C_W_R((uint32_t) obj->slave_mq[obj->fifo_rx_idx].data | 0x80000000, device->base + AST_I2CS_RX_DMA); AST1030
							I2C_W_R(TO_PHY_ADDR(obj->slave_mq[obj->fifo_rx_idx].data), device->base + AST_I2CS_RX_DMA);
						}
					}
					I2C_W_R(AST_I2CS_SET_RX_DMA_LEN(I2C_SLAVE_BUFF_SIZE), device->base + AST_I2CS_DMA_LEN);
				} else if (obj->mode == BUFF_MODE) {
			...
			case AST_I2CS_SLAVE_MATCH | AST_I2CS_RX_DONE | AST_I2CS_Wait_TX_DMA:
				...
				printf("not go to here 1\n");
				cmd = AST_I2CS_ACTIVE_ALL | AST_I2CS_PKT_MODE_EN;
				if (obj->mode == DMA_MODE) {
					cmd |= AST_I2CS_TX_DMA_EN;
					slave_rx_len = AST_I2C_GET_RX_DMA_LEN(readl(device->base + AST_I2CS_DMA_LEN_STS));
#ifdef CONFIG_I2C_MQUEUE_SLAVE	
					printf("not go to here 2\n");
					slave_rx_idx = obj->slave_mq[obj->fifo_rx_idx].length;
					for (i = 0; i < slave_rx_len; i++) {
					...
					obj->fifo_rx_idx %= I2C_SLAVE_MQ_FIFO_SIZE;
#endif
					I2C_W_R(AST_I2CC_SET_TX_BUF_LEN(obj->buf_size), device->base + AST_I2CC_BUFF_CTRL);
				} else {
					cmd &= ~AST_I2CS_PKT_MODE_EN;
					cmd |= AST_I2CS_TX_CMD;
					byte_data = AST_I2CC_GET_RX_BUFF(readl(device->base + AST_I2CC_STS_AND_BUFF));
					...
					I2C_W_R(byte_data, device->base + AST_I2CC_STS_AND_BUFF);
				}
				I2C_W_R(cmd, device->base + AST_I2CS_CMD_STS);
			...
			case AST_I2CS_RX_DONE | AST_I2CS_Wait_TX_DMA:
				...
				if (obj->mode == DMA_MODE) {
					...
					I2C_W_R(0, device->base + AST_I2CS_DMA_LEN_STS);				
					I2C_W_R(AST_I2CS_SET_TX_DMA_LEN(1), device->base + AST_I2CS_DMA_LEN);
				} else if (obj->mode == BUFF_MODE) {
					cmd |= AST_I2CS_TX_BUFF_EN;
					slave_rx_len = AST_I2CC_GET_RX_BUF_LEN(readl(device->base + AST_I2CC_BUFF_CTRL));
					...
					I2C_W_R(AST_I2CC_SET_TX_BUF_LEN(1), device->base + AST_I2CC_BUFF_CTRL);
				} else {
					cmd &= ~AST_I2CS_PKT_MODE_EN;
					cmd |= AST_I2CS_TX_CMD;
					byte_data = AST_I2CC_GET_RX_BUFF(readl(device->base + AST_I2CC_STS_AND_BUFF));
			...
			case AST_I2CS_SLAVE_MATCH | AST_I2CS_Wait_TX_DMA:
				...
				if (obj->mode == DMA_MODE) {
					...
					I2C_W_R(AST_I2CS_SET_TX_DMA_LEN(obj->slave_tx_buf[0]), device->base + AST_I2CS_DMA_LEN);
				} else if (obj->mode == BUFF_MODE) {
					cmd |= AST_I2CS_TX_BUFF_EN;
					i2c_slave_event(obj, I2C_SLAVE_READ_REQUESTED);
					...
					I2C_W_R(AST_I2CC_SET_TX_BUF_LEN(byte_data), device->base + AST_I2CC_BUFF_CTRL);
				} else {
					cmd &= ~AST_I2CS_PKT_MODE_EN;
					cmd |= AST_I2CS_TX_CMD;
					i2c_slave_event(obj, I2C_SLAVE_READ_REQUESTED);
			...
			case AST_I2CS_Wait_TX_DMA:
				...
				if (obj->mode == DMA_MODE) {
					...
					I2CS_DBUG("tx : [%02x]", obj->slave_tx_buf[0]);
					I2C_W_R(0, AST_I2CS_DMA_LEN_STS);
					I2C_W_R(AST_I2CS_SET_TX_DMA_LEN(1), device->base + AST_I2CS_DMA_LEN);
				} else if (obj->mode == BUFF_MODE) {	
					cmd |= AST_I2CS_TX_BUFF_EN;
					i2c_slave_event(obj, I2C_SLAVE_READ_PROCESSED);
			...
			case AST_I2CS_TX_NAK | AST_I2CS_STOP:
				...
				if (obj->mode == DMA_MODE) {
					cmd |= AST_I2CS_RX_DMA_EN;
					I2C_W_R(0, device->base + AST_I2CS_DMA_LEN_STS);
					I2C_W_R(AST_I2CS_SET_RX_DMA_LEN(I2C_SLAVE_MSG_BUF_SIZE), device->base + AST_I2CS_DMA_LEN);
				} else if (obj->mode == BUFF_MODE) {
					cmd |= AST_I2CS_RX_BUFF_EN;
					I2C_W_R(AST_I2CC_SET_RX_BUF_LEN(obj->buf_size), device->base + AST_I2CC_BUFF_CTRL);
				} else {
			...
			case AST_I2CS_SLAVE_MATCH | AST_I2CS_Wait_RX_DMA:
				...
				I2C_W_R(AST_I2CS_SET_RX_DMA_LEN(I2C_SLAVE_MSG_BUF_SIZE), device->base + AST_I2CS_DMA_LEN);
				I2C_W_R(readl(device->base + AST_I2CS_CMD_STS) | AST_I2CS_RX_DMA_EN, device->base + AST_I2CS_CMD_STS);
				break;
			...
			case AST_I2CS_RX_DONE | AST_I2CS_Wait_TX_DMA:
				...
				I2C_W_R(AST_I2CS_SET_TX_DMA_LEN(1), device->base + AST_I2CS_DMA_LEN);
				I2C_W_R(readl(device->base + AST_I2CS_CMD_STS) | AST_I2CS_RX_DMA_EN | AST_I2CS_TX_BUFF_EN, device->base + AST_I2CS_CMD_STS);
				break;
#endif
			default:
		...
		I2C_W_R(AST_I2CS_PKT_DONE, device->base + AST_I2CS_ISR);
		ret = 1;
	} else {
...
//				i2c_slave_event(obj, I2C_SLAVE_READ_REQUESTED);
				I2CS_DBUG("tx: [%02x]\n", byte_data);
				I2C_W_R(byte_data, AST_I2CC_STS_AND_BUFF);
				break;
...
//				i2c_slave_event(obj, I2C_SLAVE_STOP);
		...
		I2C_W_R(cmd, device->base + AST_I2CS_CMD_STS);
		I2C_W_R(sts, device->base + AST_I2CS_ISR);
		ret = 1;
	}
...
int i2c_slave_mqueue_read(i2c_t *obj, uint8_t *data)
{
	...
	if(obj->fifo_rx_idx != obj->fifo_fetch_idx) {
		...
		if(obj->fifo_full) {
			obj->fifo_rx_idx++;
			obj->fifo_rx_idx %= I2C_SLAVE_MQ_FIFO_SIZE;
//			I2C_W_R((uint32_t) obj->slave_mq[obj->fifo_rx_idx].data | 0x80000000, device->base + AST_I2CS_RX_DMA);
			I2C_W_R(TO_PHY_ADDR(obj->slave_mq[obj->fifo_rx_idx].data), device->base + AST_I2CS_RX_DMA);
			I2C_W_R(AST_I2CS_SET_RX_DMA_LEN(I2C_SLAVE_BUFF_SIZE), device->base + AST_I2CS_DMA_LEN);
			obj->fifo_full = 0;
			I2C_W_R(AST_I2CS_ACTIVE_ALL | AST_I2CS_PKT_MODE_EN | AST_I2CS_RX_DMA_EN, device->base + AST_I2CS_CMD_STS);
			I2C_W_R(AST_I2CS_PKT_DONE, device->base + AST_I2CS_IER);
		}
		return length;
	} else
		return 0;
...
void i2c_slave_mode(i2c_t *obj, int enable_slave)
{
	...
	if(enable_slave) {
		...
		if (obj->mode == BYTE_MODE){
			I2C_W_R(0xffff, device->base + AST_I2CS_IER);
		} else {
...
//		printf("slave enable : isr %x isr %x \n", readl(device->base + AST_I2CM_ISR), readl(device->base + AST_I2CS_ISR));
		...
		if(obj->mode == DMA_MODE) {
			...
			obj->fifo_rx_idx = 0;
			I2C_W_R(TO_PHY_ADDR(obj->slave_mq[0].data), device->base + AST_I2CS_RX_DMA);
//			I2C_W_R((uint32_t) obj->slave_mq[0].data | 0x80000000, device->base + AST_I2CS_RX_DMA); AST1030
			I2C_W_R(AST_I2CS_SET_RX_DMA_LEN(I2C_SLAVE_BUFF_SIZE), device->base + AST_I2CS_DMA_LEN);
#endif
		} else if (obj->mode == BUFF_MODE) {
			cmd |= AST_I2CS_RX_BUFF_EN;
		...
		I2C_W_R(AST_I2CS_AUTO_NAK_EN, device->base + AST_I2CS_CMD_STS);
		I2C_W_R(AST_I2CC_SLAVE_EN | readl(device->base + AST_I2CC_FUN_CTRL), device->base + AST_I2CC_FUN_CTRL);
		I2C_W_R(cmd, device->base + AST_I2CS_CMD_STS);
	} else {
		I2C_W_R(~AST_I2CC_SLAVE_EN & readl(device->base + AST_I2CC_FUN_CTRL), device->base + AST_I2CC_FUN_CTRL);
	}
...
void i2c_slave_address(i2c_t *obj, int idx, uint8_t address, uint8_t enable)
{
	...
	switch(idx) {
		case 0:			
			mask_addr = (readl(device->base + AST_I2CS_ADDR_CTRL) & ~(AST_I2CS_ADDR1_CLEAR));
			if(enable)
				mask_addr |= (AST_I2CS_ADDR1(address)|AST_I2CS_ADDR1_ENABLE);		
			break;
		case 1:			
			mask_addr = (readl(device->base + AST_I2CS_ADDR_CTRL) & ~(AST_I2CS_ADDR2_CLEAR));
			if(enable)
				mask_addr |= (AST_I2CS_ADDR2(address)|AST_I2CS_ADDR2_ENABLE);
			break;
		case 2:			
			mask_addr = (readl(device->base + AST_I2CS_ADDR_CTRL) & ~(AST_I2CS_ADDR3_CLEAR));
			if(enable)			
				mask_addr |= (AST_I2CS_ADDR3(address)|AST_I2CS_ADDR3_ENABLE);
			break;			
	}
	
	// fire in register.	
	I2C_W_R(mask_addr, device->base + AST_I2CS_ADDR_CTRL);
}
...
{
	//uint32_t snoop_cmd = (AST_I2CS_SNOOP_EN |AST_I2CS_SNOOP_LOOP|AST_I2CS_RX_DMA_EN|AST_I2CS_TX_DMA_EN|AST_I2CS_PKT_MODE_EN);
	uint32_t snoop_cmd = (AST_I2CS_SNOOP_EN |AST_I2CS_SNOOP_LOOP|AST_I2CS_RX_DMA_EN|AST_I2CS_PKT_MODE_EN);
	aspeed_device_t *device = obj->device;
	
	if(obj->snoop_buf == NULL){
		obj->snoop_buf = pvPortMallocNc(I2C_SNOOP_BUFF_SIZE+ALIGN_SNOOP);
		if(obj->snoop_buf == NULL){
			printf("====== i2c snoop could not allocte NC buffer size 0x%x ====== \n",I2C_SNOOP_BUFF_SIZE);
			printf("====== ABORT ====== \n");
			return;
		}		
		obj->snoop_align_buf = (uint8_t *)((uint32_t)(obj->snoop_buf) & ~ALIGN_SNOOP);
		obj->snoop_read_pos = (size_t)(obj->snoop_align_buf);		
		obj->snoop_buf_max = obj->snoop_read_pos + I2C_SNOOP_BUFF_SIZE;
		
		printf("snoop base : %x\n",obj->snoop_buf);
		printf("snoop align base : %x\n",obj->snoop_align_buf);	
		printf("snoop max : %x\n",obj->snoop_buf_max); 
	}
	...
	if(enable) {
		...
		I2C_W_R(0, device->base + AST_I2CS_IER);
		//I2C_W_R(snoop_cmd, device->base + AST_I2CS_CMD_STS);
		I2C_W_R(TO_PHY_ADDR(obj->snoop_align_buf), device->base + AST_I2CS_RX_DMA);		
		//I2C_W_R(TO_PHY_ADDR(obj->snoop_buf), device->base + AST_I2CS_RX_DMA);
		printf("en sp RX base: %x\n", readl(device->base + AST_I2CS_RX_DMA));
		I2C_W_R(0, device->base + AST_I2CS_SNOOP_DMA_RPT);
		//I2C_W_R(TO_PHY_ADDR(obj->snoop_buf), device->base + AST_I2CS_SNOOP_DMA_RPT);
		//I2C_W_R(TO_PHY_ADDR(obj->snoop_buf)-2, device->base + AST_I2CS_SNOOP_DMA_RPT);
		//I2C_W_R(TO_PHY_ADDR(obj->snoop_buf_max)+2, device->base + AST_I2CS_SNOOP_DMA_RPT);
		printf("fill sp read initial base: %x\n", readl(device->base + AST_I2CS_SNOOP_DMA_RPT));
		printf("read sp write initial base: %x\n", readl(device->base + AST_I2CS_SNOOP_DMA_WPT));
		I2C_W_R(AST_I2CS_SET_RX_DMA_LEN(I2C_SNOOP_BUFF_SIZE), device->base + AST_I2CS_DMA_LEN);		
		printf("en sp RX length: %x\n", readl(device->base + AST_I2CS_DMA_LEN));
		I2C_W_R(snoop_cmd, device->base + AST_I2CS_CMD_STS);
		printf("sts addr: %x\n", device->base + AST_I2CS_CMD_STS);
		printf("sts read back: %x\n", readl(device->base + AST_I2CS_CMD_STS));
		printf("read sp write initial base after set: %x\n", readl(device->base + AST_I2CS_SNOOP_DMA_WPT));
	} else {
		// reset whole behavior.
		I2C_W_R(~(snoop_cmd) & readl(device->base + AST_I2CS_CMD_STS), device->base + AST_I2CS_CMD_STS);		
		// reset dma offset
		I2C_W_R(~AST_I2CC_MASTER_EN & readl(device->base + AST_I2CC_FUN_CTRL), device->base + AST_I2CC_FUN_CTRL);		
		I2C_W_R((readl(device->base + AST_I2CC_FUN_CTRL)|AST_I2CC_MASTER_EN), device->base + AST_I2CC_FUN_CTRL);
		obj->snoop_read_pos = 0;
		obj->snoop_buf_max = 0;
		obj->snoop_buf_base = 0;
		vPortFreeNc(obj->snoop_buf);
		obj->snoop_buf = NULL;		
		obj->snoop_align_buf = NULL;
	}
	return;
}

void i2c_snoop_print(i2c_t *obj, size_t byte_size, uint8_t begin)
{
	uint8_t *buff = (uint8_t *)(obj->snoop_read_pos);
	size_t i = 0;
...
void i2c_snoop_read(i2c_t *obj, size_t byte_size)
{
	...
	if(readl(device->base + AST_I2CS_SNOOP_DMA_RPT) != readl(device->base + AST_I2CS_SNOOP_DMA_WPT)) {
		...
		if(obj->snoop_read_pos + byte_size <= obj->snoop_buf_max){ // normal case, just update
		
#ifdef PRINT_SNOOP // Simulate read
			i2c_snoop_print(obj, byte_size, 0);
#endif		
			//I2C_W_R(readl(device->base + AST_I2CS_SNOOP_DMA_WPT) + byte_size, device->base + AST_I2CS_SNOOP_DMA_RPT);
			//I2C_W_R(readl(device->base + AST_I2CS_SNOOP_DMA_WPT) -2, device->base + AST_I2CS_SNOOP_DMA_RPT);
			//I2C_W_R(readl(device->base + AST_I2CS_SNOOP_DMA_WPT), device->base + AST_I2CS_SNOOP_DMA_RPT);
			I2C_W_R(0, device->base + AST_I2CS_SNOOP_DMA_RPT);

			printf("normal sp R: %x\n", readl(device->base + AST_I2CS_SNOOP_DMA_RPT));

			obj->snoop_read_pos += byte_size;
		} else { // loop case
			still_size = obj->snoop_buf_max - obj->snoop_read_pos;			
			printf("still_size1 : %x\n",still_size);	
#ifdef PRINT_SNOOP // Simulate read
			i2c_snoop_print(obj, still_size, 0);
#endif		
			still_size = byte_size - still_size;

			printf("still_size2 : %x\n",still_size);			
			printf("loop into start : %x\n",(size_t)obj->snoop_buf+still_size);
#ifdef PRINT_SNOOP // Simulate read
			i2c_snoop_print(obj, still_size, 1);
#endif		
			obj->snoop_read_pos = (size_t)(obj->snoop_buf) + still_size;

			//I2C_W_R(readl(device->base + AST_I2CS_SNOOP_DMA_WPT) -2, device->base + AST_I2CS_SNOOP_DMA_RPT);
			//I2C_W_R(readl(device->base + AST_I2CS_SNOOP_DMA_WPT), device->base + AST_I2CS_SNOOP_DMA_RPT);
			I2C_W_R(0, device->base + AST_I2CS_SNOOP_DMA_RPT);

			//I2C_W_R(TO_PHY_ADDR(obj->snoop_buf)+still_size, device->base + AST_I2CS_RX_DMA);
			printf("loop sp R: %x\n", readl(device->base + AST_I2CS_SNOOP_DMA_RPT));
			
		}
			printf("R 0x54: %x\n", readl(device->base + 0x54));			
			printf("R 0x28: %x\n", readl(device->base + AST_I2CS_CMD_STS));
...
void i2c_mailbox_address(i2c_t *obj, int idx, uint8_t offset_type, uint8_t address, uint8_t enable)
{
	...
	switch(idx) {
		case 0:
			mask_addr = (readl(device->base + AST_I2CS_ADDR_CTRL) & ~(AST_I2CS_ADDR1_CLEAR));
			if(enable)
				mask_addr |= (AST_I2CS_ADDR1(address)|AST_I2CS_ADDR1_MBX_TYPE(offset_type)|AST_I2CS_ADDR1_ENABLE);					
			break;
		case 1:
			mask_addr = (readl(device->base + AST_I2CS_ADDR_CTRL) & ~(AST_I2CS_ADDR2_CLEAR));
			if(enable)
				mask_addr |= (AST_I2CS_ADDR2(address)|AST_I2CS_ADDR2_MBX_TYPE(offset_type)|AST_I2CS_ADDR2_ENABLE);					
			break;
		case 2:
			mask_addr = (readl(device->base + AST_I2CS_ADDR_CTRL) & ~(AST_I2CS_ADDR3_CLEAR));
			if(enable)
				mask_addr |= (AST_I2CS_ADDR3(address)|AST_I2CS_ADDR3_MBX_TYPE(offset_type)|AST_I2CS_ADDR3_ENABLE);					
			break;
	}
...
void i2c_mailbox_mode(i2c_t *obj, int enable_slave, uint8_t internal_buf, uint32_t base, uint16_t length)
{
	...
	if(enable_slave) {
		...
		obj->mailbox_align_buf = (uint8_t *)(base);
		obj->mailbox_length = length;

		} else {
			obj->mailbox_buf = pvPortMallocNc(length+ALIGN_MAILBOX);
			if(obj->mailbox_buf == NULL){
				printf("====== i2c mailbox could not allocte NC buffer size 0x%x ====== \n",(length+ALIGN_MAILBOX));
				printf("====== ABORT ====== \n");
				return;
			}		
			obj->mailbox_align_buf = (uint8_t *)((uint32_t)(obj->mailbox_buf) & ~ALIGN_MAILBOX);
			obj->mailbox_length = length;
		}
		
		obj->internal_buf = internal_buf;
			
		// clear slave interrupt
		I2C_W_R(0, device->base + AST_I2CS_IER);
		
		if(internal_buf) {
			I2C_W_R((uint32_t)(obj->mailbox_align_buf), device->base + AST_I2CS_RX_DMA);
		} else {
			I2C_W_R(TO_PHY_ADDR(obj->mailbox_align_buf), device->base + AST_I2CS_RX_DMA);
		}	

		I2C_W_R(AST_I2CS_SET_RX_DMA_LEN(length), device->base + AST_I2CS_DMA_LEN);	
		I2C_W_R(AST_I2CC_SLAVE_MAILBOX_EN  |
				readl(device->base + AST_I2CC_FUN_CTRL), device->base + AST_I2CC_FUN_CTRL);
	} else {
		I2C_W_R(~AST_I2CC_SLAVE_MAILBOX_EN & readl(device->base + AST_I2CC_FUN_CTRL), device->base + AST_I2CC_FUN_CTRL);
		// reset dma offset
		I2C_W_R(~AST_I2CC_MASTER_EN & readl(device->base + AST_I2CC_FUN_CTRL), device->base + AST_I2CC_FUN_CTRL);		
		I2C_W_R((readl(device->base + AST_I2CC_FUN_CTRL)|AST_I2CC_MASTER_EN), device->base + AST_I2CC_FUN_CTRL);
		
		if(obj->internal_buf) {			
			vPortFreeNc(obj->mailbox_buf);
		}
		
		obj->mailbox_buf = NULL;
		obj->mailbox_align_buf = NULL;
		obj->internal_buf = 0;
		obj->mailbox_buf_base = 0;
		obj->mailbox_length = 0;
	}
	return;
}
#endif
...
void i2c_white_list(i2c_t *obj, int idx, uint32_t *white_list_table)
{
	...
	/* Set slave addr. */
	for(i = 0; i < 64; i++)
		I2C_W_R(white_list_table[i], white_list_base + 0x200 + (idx * 0x100));
}
...
static void aspeed_i2c_irq_handler(i2c_t *obj)
{
	...
	if(readl(device->base + AST_I2CC_FUN_CTRL) & AST_I2CC_SLAVE_EN) {
		I2CS_DBUG("S irq index %d\n",DEV_ID_TO_I2C_INDEX(device->dev_id));
		//I2CS_DBUG("trigger irq status %x \n", readl(0x7e7b0000));
	...
	I2C_DBUG("M irq index %d\n",DEV_ID_TO_I2C_INDEX(device->dev_id));
	//I2C_DBUG("trigger irq status %x \n", readl(0x7e7b0000));
	if(aspeed_i2c_master_irq(obj)){
...
void i2c_reset(i2c_t *obj) {
	...
	I2C_W_R(ctrl & ~(AST_I2CC_MASTER_EN | AST_I2CC_SLAVE_EN),
				device->base + AST_I2CC_FUN_CTRL);
	I2C_W_R(ctrl, device->base + AST_I2CC_FUN_CTRL);
}
...
int i2c_dma_read(i2c_t *obj, int address, uint8_t *data, int length, uint16_t flags)
{
	...
	I2C_W_R(AST_I2CM_SET_RX_DMA_LEN(xfer_len - 1), device->base + AST_I2CM_DMA_LEN);
	//I2C_W_R((uint32_t)data | 0x80000000, device->base + AST_I2CM_RX_DMA); AST1030
	I2C_W_R(TO_PHY_ADDR(data), device->base + AST_I2CM_RX_DMA);
...
int i2c_dma_write(i2c_t *obj, int address, uint8_t *data, int length, uint16_t flags)
{
	...
	if(xfer_len) {
		...
		I2C_W_R(AST_I2CM_SET_TX_DMA_LEN(xfer_len - 1), device->base + AST_I2CM_DMA_LEN);
		//I2C_W_R((uint32_t)data | 0x80000000, device->base + AST_I2CM_TX_DMA); ast1030
		I2C_W_R(TO_PHY_ADDR(data), device->base + AST_I2CM_TX_DMA);
	}
...
int i2c_buff_write(i2c_t *obj, int address, uint8_t *data, int length, uint16_t flags)
{
	...
	if(xfer_len) {
		...
		I2C_W_R(AST_I2CC_SET_TX_BUF_LEN(xfer_len), device->base + AST_I2CC_BUFF_CTRL);
		for(i = 0; i < xfer_len; i++) {
			wbuf[i % 4] = data[i];
			if (i % 4 == 3)
				I2C_W_R(*(uint32_t *)wbuf,
...
int i2c_byte_write(i2c_t *obj, int address, uint8_t *data, int length, uint16_t flags)
{
	...
	if(length) {
		cmd |= AST_I2CM_TX_CMD;
		I2C_W_R(data[0], device->base + AST_I2CC_STS_AND_BUFF);
	}
...
static uint32_t aspeed_select_i2c_clock(struct i2c_s *obj)
{
	...
	if(obj->clk_div_mode) {
		...
		base_clk1 = (obj->apb_clk*10) / ((((clk_div_reg & 0xff) + 2) * 10) / 2);
		base_clk2 = (obj->apb_clk*10) / (((((clk_div_reg >> 8) & 0xff) + 2) * 10) / 2);
		base_clk3 = (obj->apb_clk*10) / (((((clk_div_reg >> 16) & 0xff) + 2) * 10) / 2);
		base_clk4 = (obj->apb_clk*10) / (((((clk_div_reg >> 24) & 0xff) + 2) * 10)/ 2);
//		printf("base_clk1 %ld, base_clk2 %ld, base_clk3 %ld, base_clk4 %ld \n", base_clk1, base_clk2, base_clk3, base_clk4);
		//base_clk1 50000000, base_clk2 33333333, base_clk3 25000000, base_clk4 20000000
#if 0		
...
void i2c_frequency(i2c_t *obj, int hz)
{
	...
	I2C_W_R(aspeed_select_i2c_clock(obj), device->base + AST_I2CC_AC_TIMING);
}
...
uint8_t aspeed_i2c_recover_bus(struct i2c_s *obj)
{
	...
	I2C_W_R(ctrl & ~(AST_I2CC_MASTER_EN | AST_I2CC_SLAVE_EN), device->base + AST_I2CC_FUN_CTRL);
	...
	if (!(state & AST_I2CC_SDA_LINE_STS) && (state & AST_I2CC_SCL_LINE_STS)) {
		I2C_W_R(AST_I2CM_RECOVER_CMD_EN, AST_I2CM_CMD_STS);
		ret = osEventFlagsWait(obj->evt_id, 0x00000001U, osFlagsWaitAny, CONFIG_DEVICE_I2C_TIMEOUT);
		if (ret != 0x1)
		{
			log_error("osError: %d\n", ret);
...
void aspeed_i2c_bus_reset(struct i2c_s *obj)
{
	...
	//I2C Reset
	I2C_W_R(0, device->base + AST_I2CC_FUN_CTRL);	
	I2C_W_R(i2c_crtl, device->base + AST_I2CC_FUN_CTRL);
...
static void aspeed_i2c_bus_init(struct i2c_s *obj)
{
	...
	/* Enable Master Mode */
	I2C_W_R(fun_ctrl, device->base + AST_I2CC_FUN_CTRL);

	/* Set AC Timing  TODO */
	I2C_W_R(aspeed_select_i2c_clock(obj), device->base + AST_I2CC_AC_TIMING);

	//Clear Interrupt
	I2C_W_R(0xfffffff, device->base + AST_I2CM_ISR);

	/* Set interrupt generation of I2C master controller */
	I2C_W_R(AST_I2CM_PKT_DONE | AST_I2CM_BUS_RECOVER |
					AST_I2CM_SMBUS_ALT, 
					device->base + AST_I2CM_IER);
	...
	obj->fifo_fetch_idx = 0;
	obj->fifo_rx_idx = 0;
	obj->slave_mq = pvPortMallocNc(sizeof(struct i2c_slave_mqueue) * I2C_SLAVE_MQ_FIFO_SIZE);
	if(obj->slave_mq == NULL){
		printf("====== i2c mqueue slave could not allocte NC buffer size 0x%x ====== \n",(sizeof(struct i2c_slave_mqueue) * I2C_SLAVE_MQ_FIFO_SIZE));
		printf("====== ABORT ====== \n");
		return;
	}
