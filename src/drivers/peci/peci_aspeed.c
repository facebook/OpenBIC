...
static void aspeed_peci_isr(void)
{
	...
	int_pending = peci_register->interrupt_status.value & PECI_INT_MASK;
	do {
		if (int_pending & 0x1) {
			if (peci_adapter->cb[int_index] != NULL) {
			...
			osEventFlagsSet(evt_id, BIT(int_index));
#endif
		}
		int_index++;
		int_pending >>= 1;
...
void aspeed_peci_int_cb_hook(peci_t *obj, peci_int_id_t int_id, callback cb,
							 void *para)
{
	aspeed_device_t *peci = obj->device;
	aspeed_peci_priv_t *priv = (aspeed_peci_priv_t *)peci->private;
...
hal_status_t aspeed_peci_xfer(peci_t *obj, peci_xfer_msg_t *msg)
{
	...
	log_debug(
			"msg header has_aw_fcs = %d, addr= 0x%08x, tx_len = 0x%08x, rx_len = 0x%08x\n",
			msg->has_aw_fcs, msg->addr, msg->tx_len, msg->rx_len);
	/* set peci header */
	peci_register->peci_header.fields.target_address = msg->addr;
	if (msg->has_aw_fcs) {
		peci_register->peci_header.fields.enable_aw_fcs_cycle = 1;
		peci_register->peci_header.fields.write_data_length = msg->tx_len + 1;
	} else {
		peci_register->peci_header.fields.enable_aw_fcs_cycle = 0;
...
#ifdef USE_OS_FLAG_FOR_WAIT
	int ret;
	ret = osEventFlagsWait(evt_id, BIT(PECI_INT_CMD_DONE), osFlagsWaitAll,
						   CONFIG_DEVICE_PECI_CMD_TIMEOUT);
	osEventFlagsClear(evt_id, ret);
...
void aspeed_peci_clk_set(peci_t *obj, uint32_t freq)
{
	...
	priv->clk_freq = (bus_clk_rate >> (2 + clk_div_val)) / (msg_timing * 4 + 1);
	 ...
	 */
	peci_register->timing_negotiation.fields.message_timing_negotiation =
			msg_timing;
	peci_register->timing_negotiation.fields.address_timing_negotiation =
			addr_timing;
...
hal_status_t aspeed_peci_init(peci_t *obj)
{
	...
	peci_register->interrupt.value = PECI_INT_MASK;
	peci->init = 1;
#ifdef USE_OS_FLAG_FOR_WAIT
