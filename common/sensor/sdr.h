...
#define SDR_M(sensor_num) (((full_sensor_table[SnrNum_SDR_map[sensor_num]].M_tolerance & 0xC0) << 2) | full_sensor_table[SnrNum_SDR_map[sensor_num]].M)
...
{
	return (SDR_R(sensor_num) > 0) ? (((negative_ten_power[((SDR_R(sensor_num) + 1) & 0xF)] * val) % 10) > 5 ? 1 : 0) : 0;
}
