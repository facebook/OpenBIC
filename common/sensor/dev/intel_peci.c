...
static bool peci_read_retry(uint8_t cmd, uint8_t addr, uint8_t idx, uint16_t param, uint8_t rlen, uint8_t *rbuf)
{
  if (!rbuf)
    return false;
...
static bool get_cpu_tjmax(uint8_t addr, int *reading)
{
  ...
  if (peci_read_retry(PECI_CMD_RD_PKG_CFG0, addr, RDPKG_IDX_TJMAX_TEMP, param, rlen, rbuf) == false)
    return false;
...
static bool get_cpu_margin(uint8_t addr, int *reading)
{
  ...
  if (peci_read_retry(PECI_CMD_RD_PKG_CFG0, addr, RDPKG_IDX_PKG_TEMP, param, rlen, rbuf) == false)
    return false;
...
static bool get_dimm_temp(uint8_t addr, uint8_t type, int *reading)
{
  ...
  if (peci_read_retry(PECI_CMD_RD_PKG_CFG0, addr, RDPKG_IDX_DIMM_TEMP, param, rlen, rbuf) == false)
    return false;
