...
bool u50su4p180pmdafc_i2c_read(uint8_t bus, uint8_t addr, uint8_t reg, uint8_t *data, uint8_t len)
{
...
bool u50su4p180pmdafc_get_vr_status(sensor_cfg *cfg, uint8_t rail, uint8_t vr_status_rail,
				    uint16_t *vr_status)
{
	...
	case PMBUS_STATUS_WORD: {
		uint8_t data[2] = { 0 };
		if (!u50su4p180pmdafc_i2c_read(cfg->port, cfg->target_addr, PMBUS_STATUS_WORD, data,
					       sizeof(data))) {
			return false;
		}
		val = data[0] | (data[1] << 8);
	...
	case PMBUS_STATUS_VOUT: {
		uint8_t data[1] = { 0 };
		if (!u50su4p180pmdafc_i2c_read(cfg->port, cfg->target_addr, PMBUS_STATUS_VOUT, data,
					       sizeof(data))) {
			return false;
		}
		val = (uint16_t)data[0];
	...
	case PMBUS_STATUS_IOUT: {
		uint8_t data[1] = { 0 };
		if (!u50su4p180pmdafc_i2c_read(cfg->port, cfg->target_addr, PMBUS_STATUS_IOUT, data,
					       sizeof(data))) {
			return false;
		}
		val = (uint16_t)data[0];
	...
	case PMBUS_STATUS_INPUT: {
		uint8_t data[1] = { 0 };
		if (!u50su4p180pmdafc_i2c_read(cfg->port, cfg->target_addr, PMBUS_STATUS_INPUT,
					       data, sizeof(data))) {
			return false;
		}
		val = (uint16_t)data[0];
	...
	case PMBUS_STATUS_TEMPERATURE: {
		uint8_t data[1] = { 0 };
		if (!u50su4p180pmdafc_i2c_read(cfg->port, cfg->target_addr,
	...
	case PMBUS_STATUS_CML: {
		uint8_t data[1] = { 0 };
		if (!u50su4p180pmdafc_i2c_read(cfg->port, cfg->target_addr, PMBUS_STATUS_CML, data,
					       sizeof(data))) {
			return false;
		}
		val = (uint16_t)data[0];
...
bool u50su4p180pmdafc_clear_vr_status(sensor_cfg *cfg, uint8_t rail)
{
	CHECK_NULL_ARG_WITH_RETURN(cfg, false);
