...
static sys_slist_t pex90144_list;
...
static void pex90144_i2c_encode(uint32_t oft, uint8_t be, uint8_t cmd, HW_I2C_Cmd *buf);
static uint8_t pex90144_chime_read(uint8_t bus, uint8_t addr, uint32_t oft, uint8_t *resp,
				   uint16_t resp_len);
static uint8_t pex90144_chime_write(uint8_t bus, uint8_t addr, uint32_t oft, uint8_t *data,
				    uint8_t data_len);
static uint8_t pex90144_pend_for_read_valid(uint8_t bus, uint8_t addr);
static uint8_t pex90144_chime_to_axi_write(uint8_t bus, uint8_t addr, uint32_t oft, uint32_t data);
static uint8_t pex90144_chime_to_axi_read(uint8_t bus, uint8_t addr, uint32_t oft, uint32_t *resp);
static uint8_t pex90144_temp(uint8_t bus, uint8_t addr, pex_dev_t dev, uint32_t *val);
...
static uint8_t pex_dev_get(uint8_t bus, uint8_t addr, uint8_t idx, pex_dev_t *dev)
{
	...
	uint32_t resp;
	if (pex90144_access_engine(bus, addr, idx, pex_access_id, &resp)) {
				   ...
				   uint16_t resp_len)
{
	...
	HW_I2C_Cmd cmd;
	pex90144_i2c_encode(oft, 0xF, BRCM_CMD_READ, &cmd);
				    ...
				    uint8_t data_len)
{
	...
	HW_I2C_Cmd cmd;
	pex90144_i2c_encode(oft, 0xF, BRCM_CMD_WRITE, &cmd);
...
static uint8_t pex90144_pend_for_read_valid(uint8_t bus, uint8_t addr)
{
	...
	for (int i = rty; i > 0; i--) {
		if (pex90144_chime_read(bus, addr, BRCM_CHIME_AXI_CSR_CTL, (uint8_t *)&resp,
					sizeof(resp))) {
			k_msleep(10);
			continue;
		}
...
static uint8_t pex90144_chime_to_axi_write(uint8_t bus, uint8_t addr, uint32_t oft, uint32_t data)
{
	...
	if (pex90144_chime_write(bus, addr, BRCM_CHIME_AXI_CSR_ADDR, (uint8_t *)&wbuf,
				 sizeof(wbuf))) {
		goto exit;
	}
	wbuf = sys_cpu_to_be32(data);
	if (pex90144_chime_write(bus, addr, BRCM_CHIME_AXI_CSR_DATA, (uint8_t *)&wbuf,
				 sizeof(wbuf))) {
		goto exit;
	}
	wbuf = sys_cpu_to_be32(0x1); // CHIME_to_AXI_CSR Control Status: write command
	if (pex90144_chime_write(bus, addr, BRCM_CHIME_AXI_CSR_CTL, (uint8_t *)&wbuf,
				 sizeof(wbuf))) {
		goto exit;
	}
...
static uint8_t pex90144_chime_to_axi_read(uint8_t bus, uint8_t addr, uint32_t oft, uint32_t *resp)
{
	...
	if (pex90144_chime_write(bus, addr, BRCM_CHIME_AXI_CSR_ADDR, (uint8_t *)&data,
				 sizeof(data))) {
		goto exit;
	}
	data = sys_cpu_to_be32(0x2); // CHIME_to_AXI_CSR Control Status: read command
	if (pex90144_chime_write(bus, addr, BRCM_CHIME_AXI_CSR_CTL, (uint8_t *)&data,
				 sizeof(data))) {
		goto exit;
	}

	k_msleep(10);
	if (pex90144_pend_for_read_valid(bus, addr)) {
		LOG_ERR("Read data invaild");
		goto exit;
	}

	if (pex90144_chime_read(bus, addr, BRCM_CHIME_AXI_CSR_DATA, (uint8_t *)resp,
				sizeof(resp))) {
		goto exit;
	}
...
uint8_t pex90144_access_engine(uint8_t bus, uint8_t addr, uint8_t idx, pex_access_t key,
			       uint32_t *resp)
{
	CHECK_NULL_ARG_WITH_RETURN(resp, pex_api_unspecific_err);

	pex90144_unit *p = find_pex90144_from_idx(idx);
	if (!p) {
	...
	case pex_access_temp:
		if (pex90144_temp(bus, addr, p->pex_type, resp)) {
	...
	case pex_access_id:
		if (pex90144_chime_to_axi_read(bus, addr, BRCM_REG_CHIP_ID, resp)) {
	...
	case pex_access_sbr_ver:
		if (pex90144_chime_to_axi_read(bus, addr, BRCM_REG_SBR_ID, resp)) {
...
static uint8_t pex90144_temp(uint8_t bus, uint8_t addr, pex_dev_t dev, uint32_t *val)
{
	...
	if (dev == pex_dev_pcie_switch) {
		for (int8_t i = 7; i < 12; i++) {
			CmdAddr = (0x21 << 16) | (0x17 << 8) | (0x0B);
			if (pex90144_chime_to_axi_write(bus, addr, BRCM_REG_SMB_WR_CMD, CmdAddr)) {
				LOG_ERR("CHIME to AXI Write 0x%x failed", BRCM_REG_SMB_WR_CMD);
				goto exit;
			}

			if (pex90144_chime_to_axi_write(bus, addr, BRCM_REG_SMB_WR_DATA,
							i | 0x10000)) {
			...
			if (pex90144_chime_to_axi_write(bus, addr, BRCM_REG_SMB_RD_CMD, CmdAddr)) {
				LOG_ERR("CHIME to AXI Write 0x%x failed", BRCM_REG_SMB_RD_CMD);
				goto exit;
			}

			if (pex90144_chime_to_axi_read(bus, addr, BRCM_REG_SMB_RD_DATA, &resp)) {
...
pex90144_unit *find_pex90144_from_idx(uint8_t idx)
{
	...
	SYS_SLIST_FOR_EACH_NODE (&pex90144_list, node) {
		pex90144_unit *p;
		p = CONTAINER_OF(node, pex90144_unit, node);
		if (p->idx == idx) {
			return p;
		}
	}
...
uint8_t pex90144_read(sensor_cfg *cfg, int *reading)
{
	...
	switch (cfg->offset) {
	case PEX_TEMP:
		if (pex90144_access_engine(cfg->port, cfg->target_addr, p->idx, pex_access_temp,
					   reading)) {
...
uint8_t pex90144_init(sensor_cfg *cfg)
{
	...
	pex90144_init_arg *init_arg = (pex90144_init_arg *)cfg->init_args;

	pex90144_unit *p;
	p = find_pex90144_from_idx(init_arg->idx);
	if (p == NULL) {
		p = (pex90144_unit *)malloc(sizeof(pex90144_unit));
		if (!p) {
			LOG_ERR("The pex90144_unit malloc failed at index %d", init_arg->idx);
		...
		sys_slist_append(&pex90144_list, &p->node);

		if (pex_dev_get(cfg->port, cfg->target_addr, p->idx, &p->pex_type)) {
			LOG_ERR("Get pex type failed");
			sys_slist_find_and_remove(&pex90144_list, &p->node);
			SAFE_FREE(p);
			return SENSOR_INIT_UNSPECIFIED_ERROR;
		}
	}
