...
bool bmr313_get_vr_status(sensor_cfg *cfg, uint8_t rail, uint8_t vr_status_rail,
			  uint16_t *vr_status)
{
	...
	case PMBUS_STATUS_BYTE: {
		uint8_t data[1] = { 0 };
		if (!bmr313_i2c_read(cfg->port, cfg->target_addr, PMBUS_STATUS_BYTE, data,
				     sizeof(data))) {
			return false;
		}
		val = (uint16_t)data[0];
	...
	case PMBUS_STATUS_WORD: {
		uint8_t data[2] = { 0 };
		if (!bmr313_i2c_read(cfg->port, cfg->target_addr, PMBUS_STATUS_WORD, data,
				     sizeof(data))) {
			return false;
		}
		val = data[0] | (data[1] << 8);
	...
	case PMBUS_STATUS_VOUT: {
		uint8_t data[1] = { 0 };
		if (!bmr313_i2c_read(cfg->port, cfg->target_addr, PMBUS_STATUS_VOUT, data,
				     sizeof(data))) {
			return false;
		}
		val = (uint16_t)data[0];
	...
	case PMBUS_STATUS_IOUT: {
		uint8_t data[1] = { 0 };
		if (!bmr313_i2c_read(cfg->port, cfg->target_addr, PMBUS_STATUS_IOUT, data,
				     sizeof(data))) {
			return false;
		}
		val = (uint16_t)data[0];
	...
	case PMBUS_STATUS_INPUT: {
		uint8_t data[1] = { 0 };
		if (!bmr313_i2c_read(cfg->port, cfg->target_addr, PMBUS_STATUS_INPUT, data,
				     sizeof(data))) {
			return false;
		}
		val = (uint16_t)data[0];
	...
	case PMBUS_STATUS_TEMPERATURE: {
		uint8_t data[1] = { 0 };
		if (!bmr313_i2c_read(cfg->port, cfg->target_addr, PMBUS_STATUS_TEMPERATURE, data,
				     sizeof(data))) {
			return false;
		}
		val = (uint16_t)data[0];
	...
	case PMBUS_STATUS_CML: {
		uint8_t data[1] = { 0 };
		if (!bmr313_i2c_read(cfg->port, cfg->target_addr, PMBUS_STATUS_CML, data,
				     sizeof(data))) {
			return false;
		}
		val = (uint16_t)data[0];
