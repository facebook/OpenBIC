...
int i3c_attach(I3C_MSG *msg)
{
	...
	if (!dev_i3c[msg->bus]) {
		LOG_ERR("Failed to attach addr 0x%x due to bus%u did not define", msg->target_addr,
			msg->bus);
		return -ENODEV;
	}
	...
	if (ret == 0) {
		...
		LOG_ERR("Failed to attach addr 0x%x, ret: %d", msg->target_addr, ret);
	}
...
int i3c_brocast_ccc(I3C_MSG *msg, uint8_t ccc_id, uint8_t ccc_addr)
{
	...
	if (!dev_i3c[msg->bus]) {
		LOG_ERR("Failed to brocast ccc 0x%x to addr 0x%x due to bus%u did not define",
	...
	if (ret != 0) {
		LOG_ERR("Failed to brocast ccc 0x%x to addr 0x%x, ret: %d", ccc_id, ccc_addr, ret);
	}
...
int i3c_transfer(I3C_MSG *msg)
{
	...
	if (!dev_i3c[msg->bus]) {
		LOG_ERR("Failed to transfer messages to addr 0x%x due to bus%u did not define",
	...
	if (desc == NULL) {
		LOG_ERR("Failed to transfer messages to addr 0x%x due to addr did not find",
			msg->target_addr);
		return -ENODEV;
	}
