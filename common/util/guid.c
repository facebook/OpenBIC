...
uint8_t GUID_read(EEPROM_ENTRY *entry) {
  ...
  if ( (entry->offset + entry->data_len) >= (GUID_START + GUID_SIZE) ) { // Check data write out of range
    printf("GUID read out of range, type: %x, ID: %x\n",entry->config.dev_type, entry->config.dev_id);
    return GUID_OUT_OF_RANGE;
  }

  memcpy(&entry->config, &guid_config[entry->config.dev_id], sizeof(guid_config[entry->config.dev_id]));
